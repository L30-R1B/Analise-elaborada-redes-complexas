...
fn plot_degree_distribution(graph: &UnGraph<(), ()>, dir_name: &str) {
    ...
    let file_png = &format!("{}/degree_distribution.png", dir_name);
...
fn plot_network_with_centrality(graph: &UnGraph<(), ()>, dir_name: &str) {
    ...
    let file_png = &format!("{}/network_with_centrality.png", dir_name);
...
fn main() {
    ...
    if args.len() != 4 {
        eprintln!("Uso: {} <dados.txt.gz> <dir_nome> <arq_saida_nome>", args[0]);
        std::process::exit(1);
    }
    ...
    let output_file = &format!("{}/{}.env", &args[2], &args[3]);
